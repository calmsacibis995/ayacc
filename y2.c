...
/*	define s to be a terminal if t=0
	...
	if (t) {
		if( ++nnonter >= NNONTERM ) error("too many nonterminals, limit %d",NNONTERM); /* error() located in y1.c */
	...
	while( argc >= 2  && argv[1][0] == '-' ) {
		while( *++(argv[1]) ){
			...
			case 'V':
				...
				if( foutput == NULL ) error( "cannot open y.output" ); /* error() is in y1.c */
				continue;
			case 'D':
			case 'd':
...
int skipcom(){ /* skip over comments */
	int register c, i=0;  /* i is the number of lines skipped */
